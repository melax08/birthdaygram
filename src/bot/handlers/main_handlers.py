...
async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(MENU_MESSAGE, reply_markup=MAIN_BUTTONS)


async def text_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Replies to the user with a normal message."""
    await update.message.delete()
    await update.message.reply_text(TEXT_ANSWER_MESSAGE, reply_markup=MAIN_BUTTONS)
...
start_handler = MessageHandler(
    filters.Regex(HELP_BUTTON) | filters.Regex("/start") | filters.Regex("/help"), start
)
