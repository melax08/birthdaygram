...
async def delete_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        WRITE_FULL_NAME_TO_DELETE, reply_markup=ReplyKeyboardRemove()
    )
    return FULL_NAME
    ...
    if not person:
        ...
        await update.message.reply_text(
            DELETE_CONFIRMATION.format(
                person.full_name, person.birth_date.strftime(DATE_FORMAT)
            ),
...
async def _confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if answer.lower() == "нет":
        ...
        user_table = await UserTable.get_user_table(update.effective_chat.id)
        await user_table.delete_person(context.user_data.get("person_to_delete"))
        logging.info(
            USER_DELETE_LOG.format(
                get_user_info(update), context.user_data.get("person_to_delete")
            )
        )
        context.user_data.clear()
        ...
        await update.message.reply_text(REPEAT_MESSAGE, reply_markup=YES_NO_BUTTONS)
        return CONFIRMATION
...
delete_handler = ConversationHandler(
    entry_points=[
        MessageHandler(
            filters.Regex(DELETE_BUTTON) | filters.Regex("/delete"), delete_command
        )
    ],
    states={
